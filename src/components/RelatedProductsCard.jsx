...
export default function RelatedProductsCard({ productStock, productName, productPrice, productSrc, productId, productOffer }) {
    ...
    return (
        <div ref={element} className="relative w-[150px] h-[280px] mb-8 mr-2 flex flex-col justify-evenly items-center">
            {screen ?
                <div className="animate-subtitleAppear w-full h-full bg-white shadow-default flex flex-col items-center rounded-md">
                    ...
                    <Link className="w-full" href={`/products/${productId}`}>
                        <figure className="mb-2 w-[100%] h-[140px]">
                            <Image className="object-cover rounded-t-md object-center w-full h-full" width={140} height={140} priority={true} alt={productName} src={productSrc} />
                        </figure>
                    </Link>

...
export default function Filter({filterProductState, categories, subCategorySelected, setSubCategorySelected, setFilterProductState, subLevelsState, setFilterSelected, slugCategory, queryCategory }){
    ...
    return( 
        <div className={filterProductState ? "fixed w-full h-full top-0 left-0 right-0 z-[200] bg-[#000a]" : ""}>
            <aside className={filterProductState ? "fixed w-[340px] h-full translate-x-0 transition-all bg-white top-0 right-0 z-[10000] overflow-y-scroll" : "fixed w-[340px] h-full translate-x-[500px] transition-all bg-white top-0 right-0 z-[10000] overflow-y-scroll"}>
                ...
                <ul className="w-full h-auto mt-4 relative">
                    <li className="w-full p-2 flex flex-col justify-start items-start border-b border-gray-200 h-auto relative">
                        <p onClick={() => setCategoriesProductState(state => !state)} className="text-green-500 w-full text-base tracking-wider font-semibold">CATEGORIAS</p>
                        ...
                        <FontAwesomeIcon className={`absolute w-[24px] h-[24px] right-2 text-green-500 flex justify-center items-center transition-all ${categoriesProductState && `rotate-[180deg] transition-all`}`} icon={faAngleDown} />
                    </li>

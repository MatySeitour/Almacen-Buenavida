export default function OrderProducts({orderFilterState, setOrderFilterState}){
    ...
    return(
        <div className="w-auto min-w-[130px] flex absolute right-0 left-0 top-[100%] translate-y-[4px] z-10 overflow-y-scroll justify-center items-center">
            <ul className={!orderFilterState.active ? "bg-white rounded-md shadow-md w-full opacity-0 overflow-scroll max-h-0" : "bg-white rounded-md shadow-md w-full opacity-100 transition-all overflow-scroll max-h-[150px] max-w-[150px] py-2  border border-gray-200"}>
                {order.map((option) => (
                    <li 
